...
os.chdir(r'C:\Users\pc\Documents\GitHub\2001CE45_2022\proj3')

# Title of webpage
st.title("Web Based Interface for PSAT")
# Subheader
st.subheader("Input the Required Values of following:")
# st.write("constant_fk2d")
...
try:
    ...
    # Except statement on webpage
    st.write("***Please close Results_v2.csv file it is open somewhere.***")
...
def add_front_name(name,x):
    ...
    return name
print("-"*25)
# Subheader for Filtering Method
st.subheader("Choose Filtering Method From Below:")
# Radio method for choosing Filtering Method
tch_str = st.radio('Choose your Filtering Method',('1. C','2. S','3. A','4. C & S','5. C & A','6. S & A','7. C & S & A','8. all combine') )
...
# tch from tch dictionary mapping
tch = tch_dict[tch_str]

# Depending upon tch value, different inputs have been asked for user
if tch==1:
    corr = st.number_input("Enter threshold value C:", value=0)
elif tch==2:
    SNR = st.number_input("Enter threshold value S:", value=0)
elif tch==3:
    Lambda = st.number_input("Enter Lambda value for A:")
    k = st.number_input("Enter k value for A:")
    print(Lambda, k)
elif tch==4:
    corr = st.number_input('Enter thresold value C:', value=0)
    SNR = st.number_input("Enter threshold value S:", value=0)
elif tch==5:
    corr =st.number_input('Enter thresold value C:', value=0)
    Lambda =st.number_input('Enter Lambda value for A:')
...
# Subheader for replacement Method
st.subheader("Choose Replacement Method From Below:")

# Radio method for choosing sch_str
sch_str = st.radio('Choose your Replacement Method',('1. previous point','2. 2*last-2nd_last','3. overall_mean',
...
sch_dict = {
    ...
    '2. 2*last-2nd_last':2,
    '3. overall_mean':3,
    '4. 12_point_strategy':4,
    '5. mean of previous 2 point':5,
    '6. all seqential':6,
    '7. all parallel':7,

}
# Choosing sch dictionary mapping value
sch = sch_dict[sch_str]

# 'Compute' button is created for processing
    ...
    for file in files:
        ...
        if tch==7 or tch==8:
            ...
            for i in iList:
                ...
                name = add_front_name(name,i)
                store()
        # Message for User that work has been completed
